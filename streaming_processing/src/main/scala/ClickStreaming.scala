...
object ClickStreaming extends App {
  /**
  ...
  val mappedProductId = mappedUserName.transform{ rdd =>
    val productRDD = rdd.map{
      case (userName, productId) => (productId: Int) -> (userName)
    }
    productRDD.join(productNameMapRDD).map{
      case (productId, (userName, productName)) => (userName, productName)
    }
  }
